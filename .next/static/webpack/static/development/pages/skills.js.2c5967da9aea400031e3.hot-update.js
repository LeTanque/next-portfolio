...
function SmokePuff(_ref) {
  ...
  Object(react_three_fiber__WEBPACK_IMPORTED_MODULE_3__["useFrame"])(function (_ref2) {
    ...
    var delta = clock.getDelta(); // Change in clock

    console.log('delta --> ', delta);
    var s = Math.cos(tenToHundred);
    var slowAndSteady = clock.elapsedTime * (delta + 0.017) * randomFactor;
    smokePuffRef.current.scale.set(s, s, s);
    smokePuffRef.current.rotation.z = slowAndSteady;
    smokePuffRef.current.position.set(xFactor + Math.cos(tenToHundred / 30 * factor) + Math.sin(tenToHundred * 1) * factor / 10, yFactor + Math.sin(tenToHundred / 20 * factor) + Math.cos(tenToHundred * 2) * factor / 10, zFactor + Math.cos(tenToHundred / 10 * factor) + Math.sin(tenToHundred * 3) * factor / 20);
  });
  return __jsx("mesh", {
    ref: smokePuffRef,
    material: material,
...
//# sourceMappingURL=skills.js.2c5967da9aea400031e3.hot-update.js.map